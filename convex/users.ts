...
export const updateUserPhotoPermission = mutation({
    args: { id: v.string(), photoPermission: v.union(v.literal("internal+external"), v.literal("internal"), v.literal("nowhere")) },
    handler: async (ctx, { id, photoPermission }) => {
        const user = await ctx.db.query("users").filter((q) => q.eq(q.field("_id"), id)).first();
        if (!user) {
